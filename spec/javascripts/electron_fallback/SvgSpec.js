describe("Electron fallback", function() {
    ...
    it("Fallback with undefined code", function (done) {
        instance("", {width: 64, height: 64}).then(function (result) {
            console.log(result);
            done();
        });
    });
    it("Fallback with svg code", function (done) {
        instance(svgCode, {width: 64, height: 64}).then(function(result){
            console.log(result);
            done();
        });
    });
    it('Fallback with saved file', function (done) {
       fs.writeFile(filepath, svgCode, function (fsErr) {
           instance(filepath, {width: 64, height: 64}).then(function (result) {
                console.log(result);
                done();
           });
       });
